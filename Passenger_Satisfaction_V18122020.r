...
#Pegando a probabilidade de ser "SATISFIED"
...
mtx1 <- confusionMatrix(data =  Y_CLAS1, reference = Y_OBS, positive = 'satisfied')
mtx2 <- confusionMatrix(data =  Y_CLAS2, reference = Y_OBS, positive = 'satisfied')
...
hist(AUX$Y_PROB, breaks = 20)



#Curva ROC
library(pROC)
plot.roc(TRAIN_SET$Satisfaction,Y_PROB_TRAIN)

view(TRAIN_SET$Satisfaction)
...
plot(X1,Y1, type ="n", cex.axis = 1.2, cex = 0.5,
     main = 'ROC Curve', cex.main = 1.2,
     xlab = '1 - Specificity', ylab = 'Sensitivity')
lines(X1, Y1, lwd = 3, lty = 1, col = 'red') 
lines(X2, Y2, lwd = 3, lty = 1, col = 'darkBlue') 
abline(0, 1, lty = 2)
legend('bottomright',c('Train set','Test set'), lty = 1, col = c('red','darkblue'))
